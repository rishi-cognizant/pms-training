...
class Users extends Component {
  ...
  async submitNewUserForm(values, Callback){
    const { isLoggedIn , token} = this.props.getUser;
        ...
        if(response.responseBody.status == 0){
          ...
          const response1 = await fetchApi("getUserDetails", "GET", {}, 200, null);
  ...
  users() {
    ...
    return usersData.map((item, key) => {
      return (
          ...
          <td>
          <div style={{ marginTop: "5px" }}></div><Button className="btn btn-warning" onClick={() => this.editUserForm(item.user_id)} size="sm">Edit</Button>
          <div style={{ marginTop: "5px" }}></div><Button className="btn btn-danger" onClick={() => this.deleteUser(item)} size="sm" >Delete</Button>
          </td>
  ...
  render() {
    ...
    const { show, isSuccess, isError, currentPage, totalPages } = this.state;
    console.log(this.state.usersData)
    if (typeof this.state.usersData != "undefined" && this.state.usersData.length > 0)
      return (<>
        <Row>
          <div style={{ marginTop: "30px" }}>
          <EditUser editUsersData={this.state.editUsersData} user_id={this.state.user_id} showPopup={this.state.showPopup} closeEditUserForm={() => { this.closeEditUserForm() }} />
          <Col>
          ...
          >
            {({
              ...
              <form onSubmit={handleSubmit} className="mx-auto">
                ...
                {/* <Form.Group controlId="formAddress" className="formName">
