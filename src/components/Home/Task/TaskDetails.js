  ...
  async submitForm(values, Callback){
    const { isLoggedIn , token} = this.props.getUser;
        ...
        if(response.responseBody.status === 0){
          ...
          const response1 = await fetchApi("getTaskDetails", "GET", {}, 200, null);
          this.setState({ tasksData: response1.responseBody.data });
  ...
  tasks() {
    ...
    return tasksData.map((item, key) => {
      return (
          ...
          <td>
          <div style={{ marginTop: "5px" }}></div><Button className="btn btn-warning" onClick={() => this.editTaskForm(item.task_id)} style={{ marginRight: "30px" }} >Edit Task</Button>
          <div style={{ marginTop: "5px" }}></div><Button className="btn btn-danger" onClick={() => this.deleteTask(item)} size="sm" >Delete</Button>
          </td>
          </tr>
    ...
    if (typeof this.state.tasksData != "undefined" && this.state.tasksData.length > 0)
      return (<>
        <Row>
          ...
          <EditTask editTasksData={this.state.editTasksData} task_id={this.state.task_id} showPopup={this.state.showPopup} closeEditForm={() => { this.closeEditForm() }} />
